...
/// </summary>
public sealed class LocalizedErrorService(ILocalizationService localizationService) : ILocalizedErrorService
{
    public Error CreateValidationError(string code, string messageKey, string? culture = null)
    {
        var message = localizationService.GetString(messageKey, culture);
    ...
    {
        var message = localizationService.GetString(messageKey, culture, args);
        return Error.Validation(code, message);
    }
